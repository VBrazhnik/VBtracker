...
class ToDoSimpleItemTouchHelperCallback extends ItemTouchHelper.Callback {

    private static final float ALPHA_FULL = 1.0f;

    private final ItemTouchHelperAdapter mAdapter;

    ToDoSimpleItemTouchHelperCallback(ItemTouchHelperAdapter adapter) {
        mAdapter = adapter;
    }
    ...
    public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
        ...
        if (viewHolder instanceof ToDoRecyclerListAdapter.ItemViewHolderNotDone) {
            return makeMovementFlags(0, ItemTouchHelper.END);
        } else if (viewHolder instanceof ToDoRecyclerListAdapter.ItemViewHolderDone) {
